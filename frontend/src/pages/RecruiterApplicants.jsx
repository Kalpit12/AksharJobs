        ...
        for (const job of jobs) {
          ...
          const enrichedCandidates = await Promise.all(
            applications.map(async (candidate) => {
              if (!candidate.status) return null; // Skip candidates with empty status
              const userResponse = await axios.get(`http://localhost:5000/api/auth/get_user?userId=${candidate.userId}`);
              return { ...candidate, ...userResponse.data, jobTitle: job.job_title };
            })
          );
        ...
        {loading ? (
          ...
          <>
            <table className="candidates_table">
              ...
              <tbody>
                {candidates
                  .filter((c) => (!statusFilter || c.status === statusFilter) && c.status) // Only show non-empty status
                  .map((candidate) => (
