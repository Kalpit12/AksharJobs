...
// Format: { name, code (ISO 3166-1 alpha-2), dialCode, flag }
...
export const formatPhoneNumber = (countryCode, phoneNumber) => {
  ...
  // Return E.164 format: +[country_code][number]
...
export const parsePhoneNumber = (fullPhoneNumber) => {
  if (!fullPhoneNumber || !fullPhoneNumber.startsWith('+')) {
    return { countryCode: '+1', phoneNumber: fullPhoneNumber || '' };
  }
  ...
  for (const country of countryCodes) {
    if (fullPhoneNumber.startsWith(country.dialCode)) {
      return {
        ...
        phoneNumber: fullPhoneNumber.substring(country.dialCode.length),
        country: country
      };
    }
  }
  ...
  // Default fallback
  return { countryCode: '+1', phoneNumber: fullPhoneNumber.substring(1) };
};
